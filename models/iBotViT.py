...
from lib.ibotutils import trunc_normal_
...
class MultiCropWrapper(nn.Module):
    ...
    """
    def __init__(self, backbone, head=None, projection_dim = 128):
        super(MultiCropWrapper, self).__init__()
        # disable layers dedicated to ImageNet labels classification
        backbone.fc, backbone.head = nn.Identity(), nn.Identity()
        self.backbone = backbone
        
        self.projection_head = nn.Linear(self.backbone.num_features, projection_dim)
    ...
    def forward_feature(self, x):
        return self.backbone(x, return_all_tokens = False)

    def get_depth(self):
        return len(self.backbone.blocks)

    def forward(self, x, mask=None, return_backbone_feat=False, proj = False, **kwargs):
        # convert to list
        ...
        if proj:
            if self.backbone.return_all_tokens:
                proj_x = self.projection_head(output[:, 0])
            else:
                proj_x = self.projection_head(output)
                
            output_ = output_, proj_x

        return output_

    @staticmethod
    def get_lora_target_modules():
        target_component = ["attn.qkv", "attn.proj", "mlp.fc1", "mlp.fc2"]
